...
## 整合 Niantic Campfire (營火) 官方 API 與 5km 即時雷達
...
| 輸入範例 | 功能說明 |
|---|---|
| `晨報`、`今日晨報`、`每日晨報` | **【新功能】** 隨時取得今日開蛋焦點與活動速報卡片（系統亦於每日 08:00 自動推播） |
| `訂閱 晨報` / `取消訂閱 晨報` | **【新功能】** 開啟或關閉每日 08:00 晨報推播 |
| `我的訂閱` | 查看個人推播訂閱狀態（晨報開關） |
| `活動`、`今日活動`、`本週活動` | 查看官方即時進行中與即將到來的團體戰日、晚餐會、極巨星期一、社群日等活動 |
...
4. **每日 08:00 晨報定時觸發/預覽 API**：
   瀏覽器訪問預覽：`http://127.0.0.1:8000/cron/daily-digest?dry_run=true`
   外部定時排程 (如 cron-job.org) 每天 08:00 發送 GET 或 POST 至該端點觸發推播
5. **Campfire 連線診斷**：
